...
void FullFctModel::Init(char* embfile, char* trainfile, int rank)
{
    ...
    //fea_params.fea_dim = 30;
    fea_params.fea_dim = rank;
    cout << rank << endl;
...
//    else labeldict["NA"] = 0;
    ...
    //ofstream ofs("feat.sst.init.txt");
    ...
    if (num_feats == fea_params.fea_dim) fea_model -> InitEmb();
    else fea_model -> InitEmb(fea_params.fea_dim);
    //fea_model -> InitEmb("pca_fea_emb/sst.featemb.txt", fea_params.fea_dim);
...
//    feat_emb_dim = fea_params.fea_dim;
    ...
    num_models = (int)lr_fcem_list.size();
}
...
int FullFctModel::LoadInstanceInit(ifstream &ifs) {
    ...
    ((LrFcemInstance*) inst) -> Clear();
    {
        ...
        iss >> inst -> label; inst -> label = ProcLabel(inst -> label);
        ...
        iss >> inst -> ne1_beg; iss >> inst -> ne1_end; iss >> inst -> ne1_head;  
        end1 = beg1 = inst -> ne1_head;
        ...
        iss >> inst -> ne2_beg; iss >> inst -> ne2_end; iss >> inst -> ne2_head;  
        end2 = beg2 = inst -> ne2_head;
...
//                        slot_key = "FCT_in_between_" + tag;
            ...
            if (fea_params.in_mention) {
                if (count >= inst -> ne1_beg && count <= inst -> ne1_end) {
                    slot_key = "in_entity1_only";
                    fea_model -> AddFeature(slot_key);
                    AddWordFeature(slot_key, count);
                    
                    if (fea_params.word_on_path_type) {
                        if (fea_params.tag_fea && fea_params.sst) {
                            slot_key = "in_entity1_ne1\t" + inst -> ne1_types[inst -> ne1_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                        
                            slot_key = "in_entity1_ne2\t" + inst -> ne2_types[inst -> ne2_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                        }

                        if (fea_params.tag_fea && fea_params.ner) {
                            slot_key = "in_entity1_ner_pair\t" + inst -> ne1_nes[inst -> ne1_len - 1] + '\t' + inst -> ne2_nes[inst -> ne2_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                            slot_key = "in_entity1_ne1_ner\t" + inst -> ne1_nes[inst -> ne1_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                            slot_key = "in_entity1_ne2_ner\t" + inst -> ne2_nes[inst -> ne2_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                        }
                    }
                }
                if (count >= inst -> ne2_beg && count <= inst -> ne2_end) {
                    slot_key = "in_entity2_only";
                    fea_model -> AddFeature(slot_key);
                    AddWordFeature(slot_key, count);
                    
                    if (fea_params.word_on_path_type) {
                        if (fea_params.tag_fea && fea_params.sst) {
                            slot_key = "in_entity2_ne1\t" + inst -> ne1_types[inst -> ne1_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                        
                            slot_key = "in_entity2_ne2\t" + inst -> ne2_types[inst -> ne2_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                        }

                        if (fea_params.tag_fea && fea_params.ner) {
                            slot_key = "in_entity2_ner_pair\t" + inst -> ne1_nes[inst -> ne1_len - 1] + '\t' + inst -> ne2_nes[inst -> ne2_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                            slot_key = "in_entity2_ne1_ner\t" + inst -> ne1_nes[inst -> ne1_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                            slot_key = "in_entity2_ne2_ner\t" + inst -> ne2_nes[inst -> ne2_len - 1];
                            fea_model -> AddFeature(slot_key);
                            AddWordFeature(slot_key, count);
                        }
                    }
...
string FullFctModel::ProcLabel(string input_type) {
    if (coarse_rel) {
        //cout << input_type << "\t";
        string type;
        int idx = input_type.find_first_of(":");
        int idx2 = input_type.find_first_of("(");
        if(idx == -1 && idx2 == -1) type = input_type;
        if(idx == -1) type = input_type.substr(0, idx2);
        else type = input_type.substr(0, idx);
        //cout << input_type << "\t" << idx << "\t" << idx2 << "\t" << type << endl;
        string ret;
        if(idx2 == -1) ret = "";
        else ret = input_type.substr(idx2);
        if (ace_rel) {
            if (strcmp(type.c_str(), "NA") == 0)
                return type;
            else if (strcmp(type.c_str(), "PHYS") == 0)
                return type;
            else if (strcmp(type.c_str(), "PER-SOC") == 0) {
                return type;
            }
        }
        else {
        ...
        int idx = input_type.find_first_of(":");
        int idx2 = input_type.find_first_of("(");
        if(idx == -1 && idx2 == -1) coarsetype = input_type;
        if(idx == -1) coarsetype = input_type.substr(0, idx2);
        else coarsetype = input_type.substr(0, idx);
        
        if (idx2 != -1) type = input_type.substr(0, idx2);
        else type = input_type;
        
        string ret;
        if(idx2 == -1) ret = "";
        else ret = input_type.substr(idx2);
        if (ace_rel) {
            if (strcmp(coarsetype.c_str(), "NA") == 0)
                return coarsetype;
            else if (strcmp(coarsetype.c_str(), "PHYS") == 0)
                return type;
            else if (strcmp(coarsetype.c_str(), "PER-SOC") == 0) {
                return type;
            }
        }
        else {
            if (strcmp(coarsetype.c_str(), "NA") == 0)
                return coarsetype;
            else if (strcmp(type.c_str(), "social:unspecified") == 0)
                return type;
            else if (strcmp(type.c_str(), "social:family") == 0)
                return type;
            else if (strcmp(type.c_str(), "social:business") == 0)
                return type;
        }
        type = type + ret;
        return type;
...
void FullFctModel::TrainData(string trainfile, string devfile, int type) {
    if (lr_fcem_list.size() != 0) {
        ...
        printf("FeaEmb: %lf %lf %lf %lf\n", fea_model->syn0[0], fea_model->syn0[fea_params.fea_dim-1], fea_model->syn0[fea_params.fea_dim], fea_model->syn0[2 * fea_params.fea_dim]);
    }
    //ofstream ofs("feat.sst.txt");
    int count = 0;
...
void FullFctModel::EvalData(string trainfile, string outfile, int type) {
    ...
    while (LoadInstance(ifs, type)) {
        ...
        ofs << (id + total) << "\t" << labellist[max_p] << "\t" << labellist[inst -> label_id] << endl;// << "\t" << inst -> scores[max_p] << endl;
        if (max_p != 0) pos_pred ++;
        if (max_p == inst -> label_id) {
            right++;
            if (inst -> label_id != 0) tp++;
