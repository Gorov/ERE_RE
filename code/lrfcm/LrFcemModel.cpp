...
void LrFcemModel::Init(char* embfile, char* trainfile, int type)
{}

void LrFcemModel::InitModel()
{
    ...
    for (int a = 0; a < num_labels * word_emb_dim * feat_emb_dim; a++) params_g[a] = 0.1;
    //for (int a = 0; a < num_labels * word_emb_dim * feat_emb_dim; a++) params_g[a] = 0.0;
...
void LrFcemModel::OuterProd(vector<int>& words, int num_words, vector<int>& feats, int num_feats) {
    int a;
    long long l1;
...
void LrFcemModel::ForwardOuterProd(BaseInstance* b_inst)
{
...
void LrFcemModel::ForwardOutputs(BaseInstance* b_inst)
{
    int c;
    long long l1, l2;
    real sum;
...
long LrFcemModel::BackPropOuterProd(BaseInstance* b_inst, real eta_real) {
    int c, y;
    long long l1, l2;
        ...
        for (int i = 0; i < feat_emb_dim; i++) {
            ...
            for (int j = 0; j < word_emb_dim; j++) {
                ...
                if (!adagrad) {
                    ...
                    label_emb[l1 + l2 + j] += eta_real / (sqrt(params_g[l1 + l2 + j])) * ( (y - b_inst -> scores[c]) * emb_p[l2 + j]
                            - lambda * label_emb[l1 + l2 + j]);
...
void LrFcemModel::BackPropViews(vector<int>& words, int num_words, vector<int>& feats, int num_feats, real eta_real) {
    int a;
    long long l1, l2;
    for (int i = 0; i < word_emb_dim; i++) word_emb_sum[i] = 0.0;
    for (int i = 0; i < feat_emb_dim; i++) feat_emb_sum[i] = 0.0;
...
long LrFcemModel::BackPropPhrase(BaseInstance* b_inst, real eta_real) {
    LrFcemInstance* p_inst = (LrFcemInstance*) b_inst;
    vector<int> words;
    words.resize(1);
    for (int i = 0; i < p_inst->len; i++) {
...
void LrFcemModel::ForwardProp(BaseInstance* b_inst)
{
    ForwardOuterProd(b_inst);
    ForwardOutputs(b_inst);
}

void LrFcemModel::BackProp(BaseInstance* b_inst, real eta_real)
{
//    int a;
//    long long l1, l2;
...
void LrFcemModel::PrintModelInfo() {
    cout << word_emb_dim;
    cout << feat_emb_dim;
}
